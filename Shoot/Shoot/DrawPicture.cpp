...
static int DrawPicFunc(Param_t *i)
{
	...
	case 'F':
		{
			...
			if(i->Extra.IntPosOn ?
				...
				:
				DrawModiGraphF(
					(float)(i->Layout->u.Free.LTX + F_SHIFT),
					(float)(i->Layout->u.Free.LTY + F_SHIFT),
					(float)(i->Layout->u.Free.RTX + F_SHIFT),
					(float)(i->Layout->u.Free.RTY + F_SHIFT),
					(float)(i->Layout->u.Free.RBX + F_SHIFT),
					(float)(i->Layout->u.Free.RBY + F_SHIFT),
					(float)(i->Layout->u.Free.LBX + F_SHIFT),
					(float)(i->Layout->u.Free.LBY + F_SHIFT),
					grphHdl,
					1
					)
				) // ? Ž¸”s
	...
	if(i->Extra.AlphaOn || i->Extra.BlendAddOn || i->Extra.InvOn)
	{
		ResetBlend();
	}
	if(i->Extra.MosaicOn)
